...
export default function(state: State = initState, action) {
  ...
  if (action.type === CONSTANT.ATTENDANCE.INSERT_NEW_STUDENT) {
    console.log("student", state.studentsByIdCourse, action.data);
    return {
      ...state,
      studentsByIdCourse: [...state.studentsByIdCourse, action.data]
    };
  }
  return state;
}
